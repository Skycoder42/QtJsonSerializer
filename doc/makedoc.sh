...
echo "QHP_NAMESPACE = \"de.skycoder42.qtjsonserializer.$verTag\"" >> $doxyRes
echo "QHP_CUST_FILTER_NAME = \"JsonSerializer $version\"" >> $doxyRes
echo "QHP_CUST_FILTER_ATTRS = \"qtjsonserializer $version\"" >> $doxyRes
echo "QHG_LOCATION = \"$qtBins/qhelpgenerator\"" >> $doxyRes
echo "INCLUDE_PATH += \"$qtHeaders\"" >> $doxyRes
echo "GENERATE_TAGFILE = \"$destDir/qtjsonserializer/qtjsonserializer.tags\"" >> $doxyRes
if [ "$DOXY_STYLE" ]; then
